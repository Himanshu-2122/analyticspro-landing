...
const navItems = [
  ...
  { label: "Analytics", href: "/dashboard/analytics", icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 13.5l5-5 4 4 5-6 4 3" /></svg> },
  { label: "A/B Tests", href: "/dashboard/ab-tests", icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 7.5 7.5 3m0 0L12 7.5M7.5 3v13.5m13.5 0L16.5 21m0 0L12 16.5m4.5 4.5V7.5" /></svg> },
  { label: "SEO", href: "/dashboard/seo", icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" /></svg> },
  { label: "Reports", href: "/dashboard/reports", icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg> },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!payload) redirect("/?auth=required");

  return (
    <div className="min-h-screen bg-[#020817] flex">
      {/* Sidebar */}
      <aside className="hidden md:flex w-60 shrink-0 flex-col fixed top-0 bottom-0 left-0 border-r border-white/[0.06] bg-[#020817] z-40">
        <div className="flex items-center gap-2.5 px-5 py-5 border-b border-white/[0.06]">
          ...
          <span className="font-heading font-bold text-white text-sm">Analytics<span className="text-violet-400">Pro</span></span>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-white hover:bg-white/[0.05] text-sm font-medium transition-all group"
            >
        ...
        <div className="p-3 border-t border-white/[0.06]">
          <div className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-white/[0.05] transition-all cursor-pointer">
            ...
            <div className="min-w-0">
              ...
              <div className="text-slate-500 text-xs truncate">{payload.email}</div>
            </div>
          </div>
          <Link href="/" className="flex items-center gap-2 px-3 py-2 mt-1 text-slate-500 hover:text-slate-300 text-xs rounded-xl hover:bg-white/[0.04] transition-all">
      ...
      {/* Main */}
      <div className="flex-1 md:ml-60 min-h-screen">{children}</div>
    </div>
  );
}
